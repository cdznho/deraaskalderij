...
SRC = ROOT / "assets" / "sources" / "020-club-brugge-stadion.jpg"
OUT = ROOT / "assets" / "posts" / "020-club-brugge-stadiondossier.png"
...
def draw_stamp(draw):
    draw.rectangle((70, 725, 300, 765), fill=ORANGE)
    draw.text((92, 733), "NIEUW STADION", font=font(ARIAL_BOLD, 20), fill=INK)
    draw.rectangle((730, 725, 1010, 779), fill=(17, 17, 17, 225))
    draw.text((752, 739), "SEIZOEN 28/29", font=font(ARIAL_BOLD, 24), fill=PAPER)
    draw.ellipse((962, 737, 992, 767), fill=BLUE)
...
def main():
    ...
    base = base.filter(ImageFilter.UnsharpMask(radius=1.0, percent=115, threshold=3))

    img = cover_crop(base, (W, H), vertical_focus=0.48, horizontal_focus=0.5)
    ...
    draw.rectangle((936, 58, 1018, 96), fill=BLUE)
    draw.rectangle((977, 58, 1018, 96), fill=INK)
    draw.ellipse((974, 70, 1006, 102), fill=ORANGE)
    draw_stamp(draw)

    headline = "Club Brugge krijgt eindelijk groen licht, maar twijfelt nu of het wel de moeite was"
    ...
    deck = "Na twintig jaar procederen begon Jan Breydel plots bijna gezellig te lijken."
    deck_y = y + 54
