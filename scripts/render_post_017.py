...
SRC = ROOT / "assets" / "sources" / "017-mexico-team-real.jpg"
OUT = ROOT / "assets" / "posts" / "017-mexico-sympathiepunten.png"
...
def draw_stamp(draw):
    draw.rectangle((70, 725, 312, 765), fill=ORANGE)
    draw.text((92, 733), "SYMPATHIEPUNTEN", font=font(ARIAL_BOLD, 20), fill=INK)
    draw.rectangle((736, 662, 1010, 716), fill=(17, 17, 17, 205))
    draw.text((758, 676), "KWARTFINALE?", font=font(ARIAL_BOLD, 24), fill=PAPER)
    draw.ellipse((962, 674, 992, 704), fill=ORANGE)
...
def main():
    ...
    img = cover_crop(base, (W, H), vertical_focus=0.36, horizontal_focus=0.5)
    draw = ImageDraw.Draw(img, "RGBA")

    draw.rectangle((0, 0, W, 152), fill=(17, 17, 17, 255))
    draw.rectangle((0, 780, W, H), fill=(17, 17, 17, 238))
    draw.rectangle((0, 152, W, 250), fill=(0, 0, 0, 35))
    draw.rectangle((0, 690, W, 780), fill=(0, 0, 0, 70))

    draw.text((70, 58), "SPORT", font=font(ARIAL_BOLD, 32), fill=WHITE)
    draw.ellipse((980, 58, 1018, 96), fill=ORANGE)
    draw_stamp(draw)

    headline = "Mexico wil kwartfinaleplaats kopen met alle sympathie die het kreeg"
    ...
    deck_y = y + 54
    deck = "Daar koop je niets mee, wordt vaak gezegd. Mexico wil dat nu testen."
