...
SRC = ROOT / "assets" / "sources" / "016-paraguay-france-scrum.png"
OUT = ROOT / "assets" / "posts" / "016-paraguay-duelkracht.png"
...
def main():
    ...
    base = base.filter(ImageFilter.UnsharpMask(radius=1.1, percent=120, threshold=3))

    bg = cover_crop(base, (W, H), vertical_focus=0.48, horizontal_focus=0.52)
    ...
    draw.rectangle((0, 152, W, 780), fill=(0, 0, 0, 45))

    photo = base.resize((940, 506), Image.LANCZOS)
    img.paste(photo, (70, 210))
    draw.rectangle((70, 210, 1010, 716), outline=PAPER, width=6)
    draw.rectangle((70, 658, 1010, 716), fill=(17, 17, 17, 190))

    draw.text((70, 58), "SPORT", font=font(ARIAL_BOLD, 32), fill=WHITE)
    draw.ellipse((980, 58, 1018, 96), fill=ORANGE)

    draw.rectangle((70, 725, 330, 765), fill=ORANGE)
    draw.text((92, 733), "FRANKRIJK-PARAGUAY", font=font(ARIAL_BOLD, 20), fill=INK)

    headline = "Trump vraagt of Paraguay ook in zijn tuin komt vechten"
    ...
    deck_y = y + 54
    for line in wrap(draw, "South Lawn krijgt naar verluidt alvast grasbescherming.", deck_font, 720):
