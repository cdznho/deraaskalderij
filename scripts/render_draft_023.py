...
SRC = ROOT / "assets" / "sources" / "023-start-hier-newsroom.png"
OUT = ROOT / "assets" / "drafts" / "023-start-hier.png"
...
def headline_font(draw, text):
    for size in (61, 57, 53, 49, 45):
        face = font(GEORGIA_BOLD, size)
        lines = wrap(draw, text, face, 940)
        if len(lines) <= 4:
            return face, lines, int(size * 1.17)
    return font(GEORGIA_BOLD, 42), wrap(draw, text, font(GEORGIA_BOLD, 42), 940), 50
...
def main():
    ...
    img = cover_crop(base, (W, H)).filter(ImageFilter.GaussianBlur(radius=7))
    ...
    draw.text((70, 58), "START HIER", font=font(ARIAL_BOLD, 32), fill=WHITE)
    draw.ellipse((936, 58, 974, 96), fill=ORANGE)
    draw.ellipse((980, 58, 1018, 96), fill=YELLOW)
    draw.rectangle((70, 725, 350, 765), fill=ORANGE)
    draw.text((92, 733), "OPEN REDACTIE", font=font(ARIAL_BOLD, 20), fill=INK)
    draw.rectangle((698, 725, 1010, 779), fill=(17, 17, 17, 225))
    draw.text((720, 739), "DAGELIJKS GERAAST", font=font(ARIAL_BOLD, 22), fill=PAPER)

    headline = "Welkom bij De Raaskalderij: nieuws dat zijn das heeft omgedaan"
    ...
    deck = "Dagelijkse Vlaamse satire over alles dat net iets te serieus werd genomen."
    deck_y = y + 54
