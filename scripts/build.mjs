...
const analyticsId = 'G-SSSGV5T6QL';
...
function articlePage(post, posts) {
  ...
  return layout({ title: post.title, description: `${post.deck} Satire van De Raaskalderij.`, rootPath, content: `
    ...
    <section class="article-interactions" data-article-interactions data-article-slug="${escape(post.slug)}" hidden><div class="interaction-intro"><p class="eyebrow">LEZERSBRIEF</p><h2>Heeft u hier iets <em>overbodigs</em> over te zeggen?</h2><p>De redactie leest alles. Niet noodzakelijk met plezier.</p></div><div class="interaction-action"><button type="button" class="like-button" data-like-button aria-pressed="false"><span aria-hidden="true">+</span> Geraas <b data-like-count>0</b></button><p data-like-hint>Een keer per browser, uit principe.</p></div><div class="comments"><div class="comments-head"><p>Reacties <span data-comment-count>0</span></p><small>Na redactionele controle</small></div><div class="comments-list" data-comments-list></div><form class="comment-form" data-comment-form><label>Naam<input name="display_name" maxlength="40" autocomplete="name" required placeholder="Bij voorkeur een echte"></label><label>Uw reactie<textarea name="body" maxlength="500" required placeholder="Een compleet noodzakelijke toevoeging"></textarea></label><button type="submit">Naar de redactie <span>→</span></button><p data-comment-status aria-live="polite"></p></form></div></section>
...
await cp(join(root, 'site', 'interactions-config.js'), join(out, 'interactions-config.js'));
await cp(join(root, 'site', 'interactions.js'), join(out, 'interactions.js'));
