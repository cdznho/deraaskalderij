...
SRC = ROOT / "assets" / "posts" / "013-bad-bunny-brussel-photo.png"
OUT = ROOT / "assets" / "posts" / "013-bad-bunny-brussel.png"
...
def cover_crop(img, size, vertical_focus=0.0):
...
def main():
    ...
    img = cover_crop(base, (W, H), vertical_focus=0.0)
    ...
    draw.text((70, 58), "SHOWBIZZ", font=font(ARIAL_BOLD, 32), fill=WHITE)
    draw.ellipse((980, 58, 1018, 96), fill=ORANGE)

    headline = "Brussel maakt noodplan voor fans die plots Bad Bunny-Spaans spreken"
    ...
    draw.line((70, y + 20, 1010, y + 20), fill=YELLOW, width=5)
    deck_font = font(ARIAL, 28)
    deck_y = y + 54
    for line in wrap(draw, "MIVB verwacht vertraging aan halte 'Benito'.", deck_font, 720):
